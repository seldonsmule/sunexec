...
func help(){
  ...
  fmt.Println("-cmd cronentries -sunrisecmd xxx -sunsetcmd yyy");
...
func printCronEntries(pW *WeatherConf, risecmd string, setcmd string){

  timeRise, timeSet := getWeatherFlowSunTimes(pW)

  timeNow := time.Now()

  fmt.Println("## SunRise SunSet entries for a crontab file - begin")
  fmt.Printf("# Autogenerated - see sunexec to change: %d/%d/%d-%d:%d:%d\n", 
  ...
  fmt.Printf("%d %d * * * %s\n", min, hour, setcmd)
  fmt.Println("#")
  fmt.Println("## SunRise SunSet entries for a crontab file - end")
...
*/
    ...
    case "cronentries":
      printCronEntries(WConf, *sunrisecmdPtr, *sunsetcmdPtr)
