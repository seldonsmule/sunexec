...
func getWeatherFlowSunTimes(pW *WeatherConf) (time.Time, time.Time) {
  ...
  if(openErr == nil){
    ...
    logmsg.Print(logmsg.Info, "found the json file of weatherinfo");
    tmpmsg := fmt.Sprintf("jsonReadFile: ", jsonfile)
    logmsg.Print(logmsg.Info, tmpmsg)
...
func help(){
  ...
  fmt.Println("-cmd exec -sunrisecmd xxx -sunsetcmd yyy");
...
func printCronEntries(pW *WeatherConf, risecmd string, setcmd string, cronoutfile string){

  bOutFile := false

  if(cronoutfile != "notset"){
    bOutFile = true
  }

  logmsg.Print(logmsg.Info, "Use cronoutfile is ", bOutFile)
  ...
  if(bOutFile){
    ...
    myfile, err = os.Create(cronoutfile)

    if(err != nil){
      fmt.Println("Error creating cronoutfile: ", cronoutfile, err)
      logmsg.Print(logmsg.Error, "Error creating cronoutfile: ", cronoutfile, err)
      return
    }

    defer myfile.Close()

    fmt.Println("Writing cron output to: ", cronoutfile)

  }else{
    myfile = os.Stdout
  }

  fmt.Fprintln(myfile, "## SunRise SunSet entries for a crontab file - begin")
  fmt.Fprintf(myfile, "# Autogenerated - see sunexec to change: %d/%d/%d-%d:%d:%d\n", 
                timeNow.Month(), timeNow.Day(), timeNow.Year(),
                timeNow.Hour(), timeNow.Minute(), timeNow.Second())
  fmt.Fprintln(myfile, "#")
  fmt.Fprintln(myfile, "# Sunrise: ", timeRise)

  hour := timeRise.Hour()
  min := timeRise.Minute()
  fmt.Fprintf(myfile, "%d %d * * * %s\n", min, hour, risecmd)

  fmt.Fprintln(myfile, "#")
  fmt.Fprintln(myfile, "# Sunset: ", timeSet)
  hour = timeSet.Hour()
  min = timeSet.Minute()
  fmt.Fprintf(myfile, "%d %d * * * %s\n", min, hour, setcmd)
  fmt.Fprintln(myfile, "#")
  fmt.Fprintln(myfile, "## SunRise SunSet entries for a crontab file - end")
...
func main() {
  ...
  cronoutfilePtr := flag.String("cronoutfile", "notset", "optional file to save when creating cron entries")
  ...
  */
  ...
  logmsg.Print(logmsg.Info, "Test for sunrise and sunset times and then execute passed in commands\n")
  ...
  logmsg.Print(logmsg.Info, "cronoutfilePtr = ", *cronoutfilePtr)
...
*/
    ...
    case "cronentries":
      printCronEntries(WConf, *sunrisecmdPtr, *sunsetcmdPtr, *cronoutfilePtr)
